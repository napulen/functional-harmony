...
training_list = [4, 11, 16, 20, 26, 31, 3, 8, 12, 17, 23, 21, 27, 29, 30, 10, 1, 2]
...
def load_notes(directory="BPS_FH_Dataset_Augmentation/", resolution=8):
    """
    ...
    pieces = [None for _ in range(18)]
    tdeviation = [None for _ in range(18)] # time deviation
    for i, n in enumerate(training_list):
        fileDir = os.path.join(directory, os.path.join(str(n+1), "notes_aug.csv"))
...
def load_chord_labels(directory="BPS_FH_Dataset_Augmentation/"):
    """
    ...
    chord_labels = [None for _ in range(18)]
    for i, n in enumerate(training_list):
        fileDir = os.path.join(directory, os.path.join(str(n+1), "chords.xlsx"))
...
def segment_pianorolls(pianorolls, tdeviation, chord_labels, resolution=8, wsize=32, hsize=4, label_type=None):
    ...
    segments_pianoroll = [[None for _ in range(18)] for _ in range(12)] # segments for 32 pieces, 12 keys
    segments_label = [[None for _ in range(18)] for _ in range(12)] # segment labels for 32 pieces, 12 keys
    for p in range(18):
...
def prepare_input_data(segments_pianoroll, segments_label, hop=32, num_steps=64, feature_size=61*32, label_type=None):
    ...
    input_segments = [[None for _ in range(18)] for _ in range(12)]
    input_labels = [[None for _ in range(18)] for _ in range(12)]
    for m in range(12):
        for p in range(18):
...
def get_augmented_training_data(label_type=None):
    """
    ...
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'BPS_FH_Dataset_Augmentation/')
    print('load data...')
    ...
    print('split data...')
    x_train, y_train = split_input_data(input_segments, input_labels)
...
if __name__ == '__main__':
    ...
    """
    [x_train, y_train] = get_augmented_training_data(label_type='chord_symbol')
